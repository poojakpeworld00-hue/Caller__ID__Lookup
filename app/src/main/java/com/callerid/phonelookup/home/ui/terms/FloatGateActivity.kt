 ...
 *
 * It never navigates; [TermsGateActivity] owns the flow. This screen just finishes
 * itself: on grant detected, after the timeout, or on tap.
 */
class FloatGateActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        pollJob = lifecycleScope.launch {
            while (isActive) {
                if (FloatKit.isGranted(this@FloatGateActivity)) {
                    finish()
                    return@launch
                }
                delay(POLL_MS)
