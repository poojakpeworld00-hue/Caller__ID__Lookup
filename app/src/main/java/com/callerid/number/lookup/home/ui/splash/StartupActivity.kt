package com.callerid.number.lookup.home.ui.splash
...
import com.callerid.admesh.data.OnDataReady
import com.callerid.admesh.domain.AdsVault
import com.callerid.admesh.presentation.AppOpenAdRegistry
import com.callerid.admesh.presentation.showAppRedirectPopup
import com.callerid.number.lookup.home.ui.AppCoreActivity
...
import com.callerid.number.lookup.home.launcher.activities.GreetingStepActivity
import com.callerid.number.lookup.home.launcher.helpers.LauncherFlow
import com.callerid.number.lookup.home.ui.intro.IntroRevealPolicy
import com.callerid.number.lookup.home.ui.language.LangChooserActivity
import com.callerid.number.lookup.home.ui.onboarding.PrimerActivity
import com.callerid.number.lookup.home.ui.terms.TermsGateActivity
...
class StartupActivity : CanvasActivity<ViewSplashBinding>() {
    ...
    private companion object {
        // Shared with AdBeaconActivity's tracing. Filter: `adb logcat -s SplashFlow`
    ...
    override fun initView() {
        ...
        getData(this, true, object : OnDataReady {
            override fun onSuccess() {
                ...
                AppOpenAdRegistry.loadAd(this@StartupActivity)
                dataReady.set(true)
                maybeProceed()
            }
...
//        }, WATCHDOG_TIMEOUT_MS)
    ...
    private fun launchNext() {
        ...
        // because we've navigated off the excluded StartupActivity by then, would
    ...
    private fun nextScreen(): Class<*> = when {
        ...
        IntroRevealPolicy.shouldShowLanguage(this) -> LangChooserActivity::class.java
        IntroRevealPolicy.shouldShowTerms(this) -> TermsGateActivity::class.java
        IntroRevealPolicy.shouldShowOnboarding(this) -> PrimerActivity::class.java
        else -> LauncherFlow.homeActivity()
    }
    private fun route() {
        val next = when {
            !prefs.isLanguageSelected -> LangChooserActivity::class.java
            !prefs.isTermsAccepted -> TermsGateActivity::class.java
            !prefs.isOnboardingDone -> PrimerActivity::class.java
            else -> AppCoreActivity::class.java
        }
