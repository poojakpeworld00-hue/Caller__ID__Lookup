...
import com.callerid.admesh.presentation.InAppUpdateRegistry
import com.callerid.number.lookup.home.R
import com.callerid.number.lookup.home.base.CarrierFragment
...
import com.callerid.number.lookup.home.ui.contacts.PhonebookFragment
import com.callerid.number.lookup.home.ui.lookup.CallerLookupFragment
import com.callerid.number.lookup.home.ui.recents.FeedFragment
import com.callerid.number.lookup.home.ui.terms.FloatKit
 ...
 *
 * Hosted by [com.callerid.number.lookup.home.ui.AppCoreActivity] and by the launcher's
...
class HomeCoreFragment : CarrierFragment<PanelHomeShellBinding>() {
     ...
     *
     * AppCoreActivity is always visible so it reports `true` once. The launcher fires it after
    ...
    private fun dashboardTab(): OverviewFragment? =
        tabs.firstOrNull { it.fragment is OverviewFragment }?.fragment as? OverviewFragment
    ...
    fun showLookup(number: String? = null) {
        val index = tabs.indexOfFirst { it.fragment is CallerLookupFragment }
        if (index < 0) return
        select(index)
        if (!number.isNullOrBlank()) {
            (tabs[index].fragment as? CallerLookupFragment)?.requestSearch(number)
        }
    }

    /** Switches to the Recents tab (Home's "See all" recent activity). */
    fun showRecents() {
        val index = tabs.indexOfFirst { it.fragment is FeedFragment }
     ...
     * own back handling — CanvasActivity has its back-ad callback and the launcher's Fossify base
    ...
    fun onBackPressed(): Boolean {
        ...
        val homeIndex = tabs.indexOfFirst { it.fragment is OverviewFragment }.coerceAtLeast(0)
    ...
    private fun applyTopInsetForTab(index: Int) {
        ...
        val immersive = fragment is OverviewFragment ||
            fragment is FeedFragment ||
            fragment is PhonebookFragment ||
            fragment is CallerLookupFragment
    ...
    companion object {
        ...
        fun newInstance(number: String? = null) = HomeCoreFragment().apply {
            if (!number.isNullOrBlank()) {
