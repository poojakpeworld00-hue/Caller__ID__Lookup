...
import com.callerid.number.lookup.home.permission.fsi.FullScreenAccess
import com.callerid.number.lookup.home.permission.fsi.FsiPortalActivity
import com.callerid.number.lookup.home.ui.AppCoreActivity
import com.callerid.number.lookup.home.ui.intro.IntroRevealConfig
...
class LangChooserActivity : CanvasActivity<ViewLanguageBinding>() {
    ...
    override fun initView() {
        ...
        // Current language comes from AppVault (the store CanvasActivity.applyLocale reads).
        ...
        // through to CanvasActivity's exit handler; `forwarding` blocks re-entry.
    ...
    private fun detectCountryByIp() {
        lifecycleScope.launch {
            val geo = RegionLocator.detectCountry(this@LangChooserActivity) ?: run {
        ...
        AccessEngine.check(this) {
            ...
            val intent = when {
                FullScreenAccess.shouldShowScreen(this) ->
                    FsiPortalActivity.newIntent(this, next)
    ...
    companion object {
        private const val TAG = "LangChooserActivity"
        ...
        fun newIntent(context: Context, standalone: Boolean = false): Intent =
            Intent(context, LangChooserActivity::class.java)
