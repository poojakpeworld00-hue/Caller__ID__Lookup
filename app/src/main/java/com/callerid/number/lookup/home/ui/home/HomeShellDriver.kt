...
import com.callerid.admesh.presentation.StoreUpdateListener
import com.callerid.admesh.presentation.StoreUpdateRegistry
import com.callerid.number.lookup.home.permission.PermitSheetDialog
import com.callerid.number.lookup.home.permission.fsi.FsiPermit
import com.callerid.number.lookup.home.permission.fsi.FsiSettings
import com.callerid.number.lookup.home.permission.fsi.FsiPrimerDialog
import com.callerid.number.lookup.home.permission.fsi.FsiReturnGuard
import com.callerid.number.lookup.home.services.ContactSync
 ...
 * initializer, and the fragment reaches them through [HomeShellOwner].
 *
...
class HomeShellDriver(private val host: HomeShellOwner) {
     ...
     *
     * Call this when the shell is actually on screen. AppHomeActivity calls it from
    ...
    /**
     * Shows the permission priming bottom sheet ([PermitSheetDialog]).
     *
     ...
     * [FsiPermit.shouldShowDialog] passes (SDK 14+, feature on, country allowed,
    ...
    /** Opens the FSI "Manage" page (called from [FsiPrimerDialog]). */
    fun openFsiSettings() {
    ...
    fun startOverlayPermissionFlow() {
        ...
        // On top of the page we just opened — see OverlayKit.showGuide. Deliberately outside
    ...
    companion object {
        ...
        /** Flags for the in-task reorder every [HomeShellOwner.bringHostToFront] uses. */
