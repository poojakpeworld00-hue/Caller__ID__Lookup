...
import io.launcher.home.extensions.getCellCount
import io.launcher.home.helpers.MAX_CLICK_DURATION
...
class MyAppWidgetResizeFrame(context: Context, attrs: AttributeSet, defStyle: Int) : RelativeLayout(context, attrs, defStyle) {
    ...
    private val lineDotRadius = context.resources.getDimension(R.dimen.launcher_resize_frame_dot_radius)
    ...
    fun updateFrameCoords(
        ...
        minResizeWidthCells = Math.min(context.launcherConfig.homeColumnCount, context.getCellCount(providerInfo.minResizeWidth))
        minResizeHeightCells = Math.min(context.launcherConfig.homeRowCount, context.getCellCount(providerInfo.minResizeHeight))
        redrawFrame()
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                when (dragDirection) {
                    ...
                    DRAGGING_BOTTOM -> {
                        ...
                        if (wantedBottomCellY == context.launcherConfig.homeRowCount - 1) {
                            areAllCellsFree = false
                        }
